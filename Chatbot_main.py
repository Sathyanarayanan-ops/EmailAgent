...
#             f"Summary:"
#         )
#         # Use the invoke method with a list of message dictionaries.
#         response = llm.invoke([{"role": "user", "content": prompt}])
#         # Assume that the response is a dictionary containing the key "content"
#         email_summary = response.get("content", "").strip()
#         individual_summaries.append(email_summary)
    
#     # Combine the individual summaries into one final summary.
#     combined_prompt = (
...
#     final_summary = combined_response.get("content", "").strip()
...
#     Since state["mails"] is a dictionary mapping user emails to lists of email dictionaries,
#     we first summarize for each user and then combine the user summaries.
#     """
#     mails = state["mails"]
#     user_summaries = []
    
#     for user, email_list in mails.items():
#         user_summary = iterative_summarize_emails(email_list)
#         user_summaries.append(f"Summary for {user}: {user_summary}")
    
#     combined_prompt = (
#         "Below are summaries of emails for different users:\n" +
#         "\n".join(user_summaries) +
#         "\n\nProvide a final, concise overall summary of all these emails:"
#     )
#     combined_response = llm.invoke([{"role": "user", "content": combined_prompt}])
#     final_summary = combined_response.get("content", "").strip()
